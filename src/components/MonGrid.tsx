...
export default function MonGrid({ mons }: { mons: Mon[] }) {
  ...
  const allTypes = useMemo(() => {
    ...
    mons.forEach(m => m.types.forEach(t => s.add(t)));
    return ['all', ...Array.from(s).sort()];
  }, [mons]);

  const filtered = useMemo(() => {
    const idQuery = search.startsWith('#') ? search.slice(1) : /^\d+$/.test(search) ? search : null;
    let result = mons.filter(m => {
      ...
      if (incomplete && m.types.length > 0 && m.fastMove && m.chargeMove1 && m.glRank != null) return false;
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.controls}>
        ...
        <select className={styles.select} value={shadow} onChange={e => setShadow(e.target.value as typeof shadow)}>
        ...
        <span className={styles.count}>{filtered.length} / {mons.length}</span>
      </div>
